...
{
    public partial class TelaTipoPagamento : Form
    {  
        #region: Membros da classe
        private PagamentoTipo c = new PagamentoTipo();
        ...
        private void SetGrid() {
            dgDados.DataSource = new List<PagamentoTipo>();
        ...
        private void Salvar()
        {
            ...
            if (c.id < 1)
                result = c.Request.pagamento_tipo.Insert();
            else
                result = c.Request.pagamento_tipo.Update();
        ...
        private void Excluir()
        {
            if (!Request.GetConfirm("Deseja excluir este cadastro ?"))
                return;
            var result = new ResultRequest<object>();
            result = c.Request.pagamento_tipo.Delete(c.id);
            if (!result.status)
            {
                Request.GetError(result.message);
                return;
        ...
        {
            var result = c.Request.pagamento_tipo.FindAll();
        ...
        {
            var result = c.Request.pagamento_tipo.Find(c.id);
